...
파이썬 객체 지향 프로그래밍 방식에서 싱글톤 패턴을
인스턴스가 한번의 메모리만 사용 함으로, 한개의 단일 객체가 보증하며, 단일 객체의 상태만을 공유하기 위함이라고 정의할떄
이를 모듈화 하여 사용하면 객체의 단독 사용이라는 똑같은 효과가 나오는게 아닌가요? 파이썬에서 싱글톤 패턴을 써야만 하는 이유와 
